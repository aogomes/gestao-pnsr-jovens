  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    if (!user || !user.papel) {
      throw new ForbiddenException(
        'Usuário não autenticado ou papel não definido.',
      );
    }

    const hasAccess = hasPermission(
      user.papel as PapelUsuario,
    ...
    if (
      requiredPermission.modulo === 'pessoas' &&
      targetId &&
      user.pessoaId === targetId
    ) {
      isOwnResource = true;
    }
    ...
    if (
      requiredPermission.modulo === 'usuarios' &&
      targetId &&
      user.sub === targetId
    ) {
      isOwnResource = true;
    ...
    if (
      requiredPermission.modulo === 'inscricoes' &&
      request.method === 'POST' &&
      !targetId
    ) {
      if (
        ...
        parseInt(request.body.pessoaId, 10) === user.pessoaId
      ) {
        isOwnResource = true;
      }
    }
    ...
    if (!hasAccess && !isOwnResource) {
      throw new ForbiddenException(
        `Acesso negado: Seu perfil (${user.papel}) não permite '${requiredPermission.acao}' no módulo '${requiredPermission.modulo}'.`,
      );
    }
