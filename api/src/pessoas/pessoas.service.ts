...
export class PessoasService {
  ...
  async criar(createPessoaDto: CreatePessoaDto, user: any) {
    const dados: any = {
      ...createPessoaDto,
      paroquiaId: ['ADMIN', 'AUDITOR'].includes(user.papel)
        ? createPessoaDto.paroquiaId || user.paroquiaId
        : user.paroquiaId,
    };
  ...
  obterSaldosPorEvento(transacoes: any[]) {
    const eventosMap: Record<
      number,
      { id: number; nome: string; receitas: number; despesas: number }
    > = {};
    ...
    transacoes.forEach((t: any) => {
      ...
      if (!eventosMap[t.eventoId]) {
        eventosMap[t.eventoId] = {
          id: t.eventoId,
          nome: t.evento?.nome || `Evento #${t.eventoId}`,
          receitas: 0,
          despesas: 0,
        };
      }
  ...
  async buscarTodas(user: any) {
    const where = ['ADMIN', 'AUDITOR'].includes(user.papel)
      ? { ativo: true }
      : { paroquiaId: user.paroquiaId, ativo: true };
    ...
    const eventosAtivosInfo = await this.prisma.evento.findMany({
      where: { status: 'ATIVO' },
      select: { id: true, nome: true },
    });
    const ativosIds = eventosAtivosInfo.map((e) => e.id);
    const nomeEventosMap = eventosAtivosInfo.reduce(
      (acc, ev) => ({ ...acc, [ev.id]: ev.nome }),
    ...
    const saldosGerais = await this.prisma.transacao.groupBy({
      ...
      where: {
        ...
        OR: [{ eventoId: { in: ativosIds } }, { eventoId: null }],
      },
      _sum: { valor: true },
    });
    ...
    pessoaIds.forEach((id) => {
      saldosDetalhadosMap[id] = [];
    });

    const agregacaoPorEvento: Record<
      string,
    ...
    saldosEventos.forEach((t) => {
      if (t.pessoaId !== null && t.eventoId !== null) {
        ...
        if (!agregacaoPorEvento[key])
          agregacaoPorEvento[key] = { receitas: 0, despesas: 0 };
        if (t.tipo === 'RECEITA')
          agregacaoPorEvento[key].receitas += t._sum.valor || 0;
        if (t.tipo === 'DESPESA')
          agregacaoPorEvento[key].despesas += t._sum.valor || 0;
      }
    });
  ...
  async buscarUma(id: number) {
    ...
    if ((pessoa as any).transacoes) {
      (pessoa as any).transacoes = (pessoa as any).transacoes.filter(
        (t: any) => !(t.evento && t.evento.status !== 'ATIVO'),
      );
    }
    const totalTransacoes = (pessoa as any).transacoes.reduce(
      (acc: number, t: any) => {
        if (t.tipo === 'RECEITA') return acc + t.valor;
        if (t.tipo === 'DESPESA') return acc - t.valor;
        return acc;
    ...
    const saldosDetalhados = this.obterSaldosPorEvento(
      (pessoa as any).transacoes,
    );
  ...
  async buscarMeuPerfil(identificador: string) {
    ...
    if (!pessoa) {
      throw new NotFoundException(
        'Perfil não encontrado para o identificador informado.',
      );
    }
