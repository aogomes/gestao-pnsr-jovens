...
export class InscricoesService {
  ...
  async buscarTodas(eventoId?: number) {
    ...
    return inscricoes.map((insc) => {
      ...
      return {
        ...insc,
        pessoa: { ...pessoaSemTransacoes, saldo: saldoCalculado },
  ...
  async atualizarStatus(id: number, status: any) {
    return this.prisma.$transaction(async (tx) => {
      ...
      // 2. Se o novo status for REJEITADA e houver pagamentos de débito da pessoa vinculados, realiza o estorno de partida dobrada
      if (status === 'REJEITADA' && inscricao.transacoes && inscricao.transacoes.length > 0) {
